// RMS normalization: out = x / sqrt(mean(x^2) + eps) * alpha
//
// Entry point: rms_norm_f32
...
fn rms_norm_f32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let row = gid.x;
    ...
    for (var c = 0u; c < n_cols; c = c + 1u) {
        let idx = row_base + c;
        output_buf[idx] = input0_buf[idx] / denom * input1_buf[c];
    }
}
