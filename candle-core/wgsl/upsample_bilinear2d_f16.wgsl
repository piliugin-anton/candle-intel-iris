enable f16;
// Bilinear 2D upsampling (f32). Entry point: upsample_bilinear2d_f16
...
fn upsample_bilinear2d_f16(
    ...
    var w_scale: f32;
    if (align_corners) {
        h_scale = select(0.0, f16(h_in - 1u) / f16(h_out - 1u), h_out > 1u);
        w_scale = select(0.0, f16(w_in - 1u) / f16(w_out - 1u), w_out > 1u);
    } else {
        h_scale = select(f16(h_in) / f16(h_out), 1.0 / bitcast<f32>(p.scale_h_bits), p.has_scale_h != 0u);
        w_scale = select(f16(w_in) / f16(w_out), 1.0 / bitcast<f32>(p.scale_w_bits), p.has_scale_w != 0u);
    }
    ...
    for (var tid = gid.x; tid < p.dst_numel; tid = tid + stride_wg) {
        ...
        var src_w_fp: f32;
        if (align_corners) {
            src_h_fp = h_scale * f16(dst_h);
            src_w_fp = w_scale * f16(dst_w);
        } else {
            src_h_fp = h_scale * (f16(dst_h) + 0.5) - 0.5;
            src_w_fp = w_scale * (f16(dst_w) + 0.5) - 0.5;
        }
        ...
        let weight_h = clamp(src_h_fp - f16(h0), 0.0, 1.0);
        let weight_w = clamp(src_w_fp - f16(w0), 0.0, 1.0);
