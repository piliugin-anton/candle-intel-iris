// RMS normalization (bf16): out = x / sqrt(mean(x^2) + eps) * alpha
//
// Entry point: rms_norm_bf16
...
fn rms_norm_bf16(@builtin(global_invocation_id) gid: vec3<u32>) {
    let row = gid.x;
