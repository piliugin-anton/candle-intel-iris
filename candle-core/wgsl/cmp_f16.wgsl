// Element-wise comparison kernels (f16 inputs, u8 output packed in u32 words).
//
// Entry points: eq_f16, ne_f16, lt_f16, le_f16, gt_f16, ge_f16
...
fn load_in0(idx: u32) -> f32 {
    return f32(input0_buf[buffer_index(idx, kernel_params.in0_layout)]);
}

fn load_in1(idx: u32) -> f32 {
    return f32(input1_buf[buffer_index(idx, kernel_params.in1_layout)]);
}
...
fn eq_f16(
    ...
    for (var i = gid.x; i < count; i = i + stride) {
        store_u8(i, select(0u, 1u, load_in0(i) == load_in1(i)));
    }
}
