...
fn upsample_bilinear2d_bf16(
    ...
    for (var tid = gid.x; tid < p.dst_numel; tid = tid + stride_wg) {
        ...
        let v00 = read_packed_bf16(input_buf[base + h0 * src_layout.strides[2] + w0 * src_layout.strides[3] / 2u], base + h0 * src_layout.strides[2] + w0 * src_layout.strides[3]);
        let v10 = read_packed_bf16(input_buf[base + h0 * src_layout.strides[2] + w1 * src_layout.strides[3] / 2u], base + h0 * src_layout.strides[2] + w1 * src_layout.strides[3]);
        let v01 = read_packed_bf16(input_buf[base + h1 * src_layout.strides[2] + w0 * src_layout.strides[3] / 2u], base + h1 * src_layout.strides[2] + w0 * src_layout.strides[3]);
        let v11 = read_packed_bf16(input_buf[base + h1 * src_layout.strides[2] + w1 * src_layout.strides[3] / 2u], base + h1 * src_layout.strides[2] + w1 * src_layout.strides[3]);

        let v_top = v00 * (1.0 - weight_w) + v10 * weight_w;
        let v_bottom = v01 * (1.0 - weight_w) + v11 * weight_w;
        output_buf[tid / 2u] = pack_bf16_value(output_buf[tid / 2u], tid, v_top * (1.0 - weight_h); + v_bottom * weight_h);
    }
}
