...
fn reduce_max_f32(
    ...
    var idx = start + tid;
    while (idx < stop) {
        best = max(best, reduce_load_src(idx));
        idx += REDUCE_WG_SIZE;
    }
    wg_max_val[tid] = best;
...
// Argmax along the reduced (typically last) dimension; stores the index as f32.
@compute @workgroup_size(REDUCE_WG_SIZE)
fn reduce_argmax_f32(
    ...
    let stop = min(start + chunk, reduce_params.src_elem_count);
    let tid = local_id.x;
    let last_dim = reduce_params.src_layout.dims[reduce_params.src_layout.num_dims - 1u];
    ...
    var idx = start + tid;
    while (idx < stop) {
        let v = reduce_load_src(idx);
        let local_i = idx % last_dim;
        if (!found || v > best_val) {
            best_val = v;
            best_idx = local_i;
            found = true;
...
// Argmin along the reduced (typically last) dimension; stores the index as f32.
@compute @workgroup_size(REDUCE_WG_SIZE)
