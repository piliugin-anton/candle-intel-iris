// Tiled matrix multiply with vec4 inner products from shared memory.
//
// Global→shared loads match the scalar tiled kernel; the K inner loop processes
// VEC=4 elements per iteration using dot(vec4, vec4).
//
// Entry point: matmul_tiled_vec_f32
...
fn tile_dot_vec4(ty: u32, tx: u32, k_base: u32) -> f32 {
    let a_vec = vec4<f32>(
