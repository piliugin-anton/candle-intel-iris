// Interleaved rotary positional embedding (f32).
//
// Entry point: rope_i_f32
...
fn rope_i_f32(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let c = cos_buf[rope_idx];
    let s = sin_buf[rope_idx];
    let i0 = 2u * idx;
    let i1 = i0 + 1u;
    let v0 = src_buf[i0];
    let v1 = src_buf[i1];
    output_buf[i0] = v0 * c - v1 * s;
    output_buf[i1] = v0 * s + v1 * c;
}
