enable f16;
// Layer normalization (f16): out = (x - mean) / sqrt(var + eps) * alpha + beta
//
// Entry point: layer_norm_f16
...
@group(0) @binding(3)
var<storage, read> input2_buf: array<f16>;

@group(0) @binding(4)
var<storage, read> layer_norm_params: LayerNormParams;

@compute @workgroup_size(32)
fn layer_norm_f16(@builtin(global_invocation_id) gid: vec3<u32>) {
    let row = gid.x;
    ...
    for (var c = 0u; c < n_cols; c = c + 1u) {
        let idx = row_base + c;
        let normed = (f32(input0_buf[idx]) - mean) * inv_std;
        let scaled = normed * f32(input1_buf[c]) + f32(input2_buf[c]);
