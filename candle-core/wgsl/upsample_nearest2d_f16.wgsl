enable f16;
// Nearest-neighbor 2D upsampling (f32). Entry point: upsample_nearest2d_f16
...
fn upsample_nearest2d_f16(
    ...
    for (var tid = gid.x; tid < p.dst_numel; tid = tid + stride_wg) {
        ...
        let src_h_idx = min(src_h - 1u, u32(f16(dst_h_idx) * scale_h));
        let src_w_idx = min(src_w - 1u, u32(f16(dst_w_idx) * scale_w));
