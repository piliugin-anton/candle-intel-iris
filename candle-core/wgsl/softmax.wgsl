// Fused softmax along the last dimension (contiguous row-major input).
//
// Entry point: softmax_last_dim_f32
...
fn softmax_last_dim_f32(
    ...
    for (var i = local_id; i < last_dim; i = i + WG_SIZE) {
        local_max = max(local_max, input_buf[offset + i]);
    }
    ...
    var local_sum = 0.0;
    for (var i = local_id; i < last_dim; i = i + WG_SIZE) {
        local_sum += exp(input_buf[offset + i] - row_max);
    }
    ...
    for (var i = local_id; i < last_dim; i = i + WG_SIZE) {
        output_buf[offset + i] = exp(input_buf[offset + i] - row_max) / row_sum;
    }
}
